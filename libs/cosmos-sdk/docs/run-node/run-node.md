...
Recall that `$MY_VALIDATOR_ADDRESS` is a variable that holds the address of the `my_validator` key in the [keyring](./keyring.md#adding-keys-to-the-keyring). Also note that the tokens in the SDK have the `{amount}{denom}` format: `amount` is is a 18-digit-precision decimal number, and `denom` is the unique token identifier with its denomination key (e.g. `okt`). Here, we are granting `okt` tokens, as `okt` is the token identifier used for staking in [`exchaind`](https://github.com/fibonacci-chain/fbc-social). For your own chain with its own staking denom, that token identifier should be used instead.
