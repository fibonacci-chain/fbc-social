...
ldflags = -X $(GithubTop)/fibonacci-chain/fbc-social/libs/cosmos-sdk/version.Version=$(Version) \
	-X $(GithubTop)/fibonacci-chain/fbc-social/libs/cosmos-sdk/version.Name=$(Name) \
  -X $(GithubTop)/fibonacci-chain/fbc-social/libs/cosmos-sdk/version.ServerName=$(ServerName) \
  -X $(GithubTop)/fibonacci-chain/fbc-social/libs/cosmos-sdk/version.ClientName=$(ClientName) \
  -X $(GithubTop)/fibonacci-chain/fbc-social/libs/cosmos-sdk/version.Commit=$(COMMIT) \
  -X $(GithubTop)/fibonacci-chain/fbc-social/libs/cosmos-sdk/version.CosmosSDK=$(CosmosSDK) \
  -X $(GithubTop)/fibonacci-chain/fbc-social/libs/cosmos-sdk/version.Tendermint=$(Tendermint) \
  -X "$(GithubTop)/fibonacci-chain/fbc-social/libs/cosmos-sdk/version.BuildTags=$(build_tags)" \
  -X $(GithubTop)/fibonacci-chain/fbc-social/libs/tendermint/types.MILESTONE_GENESIS_HEIGHT=$(GenesisHeight) \
  -X $(GithubTop)/fibonacci-chain/fbc-social/libs/tendermint/types.MILESTONE_MERCURY_HEIGHT=$(MercuryHeight) \
  -X $(GithubTop)/fibonacci-chain/fbc-social/libs/tendermint/types.MILESTONE_VENUS_HEIGHT=$(VenusHeight) \
  -X $(GithubTop)/fibonacci-chain/fbc-social/libs/tendermint/types.MILESTONE_VENUS1_HEIGHT=$(Venus1Height) \
  -X $(GithubTop)/fibonacci-chain/fbc-social/libs/tendermint/types.MILESTONE_VENUS2_HEIGHT=$(Venus2Height) \
  -X $(GithubTop)/fibonacci-chain/fbc-social/libs/tendermint/types.MILESTONE_VENUS3_HEIGHT=$(Venus3Height) \
  -X $(GithubTop)/fibonacci-chain/fbc-social/libs/tendermint/types.MILESTONE_VENUS4_HEIGHT=$(Venus4Height) \
  -X $(GithubTop)/fibonacci-chain/fbc-social/libs/tendermint/types.MILESTONE_VENUS5_HEIGHT=$(Venus5Height) \
  -X $(GithubTop)/fibonacci-chain/fbc-social/libs/tendermint/types.MILESTONE_EARTH_HEIGHT=$(EarthHeight) \
  -X $(GithubTop)/fibonacci-chain/fbc-social/libs/tendermint/types.MILESTONE_MARS_HEIGHT=$(MarsHeight)


ifeq ($(WITH_ROCKSDB),true)
  ldflags += -X github.com/fibonacci-chain/fbc-social/libs/tendermint/types.DBBackend=rocksdb
endif
...
build-docker-fbchainnode:
	$(MAKE) -C networks/local

# Run a 4-node testnet locally
localnet-start: localnet-stop
	@if ! [ -f build/node0/fbchaind/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/fbchaind:Z fbchain/node testnet --v 4 -o . --starting-ip-address 192.168.10.2 --keyring-backend=test ; fi
	docker-compose up -d
