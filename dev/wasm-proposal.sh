...
# cw20
# fb1eyfccmjm6732k7wp4p6gdjwhxjwsvje44j0hfx8nkgrm8fs7vqfsfxfyxv
# cw4-stake
# fb1fyr2mptjswz4w6xmgnpgm93x0q4s4wdl6srv3rtz3utc4f6fmxeqn3c0pp
...
FBCHAIN_DEVNET_VAL_ADMIN_MNEMONIC=(
"${ADMIN0_MNEMONIC}"
"${ADMIN1_MNEMONIC}"
"${ADMIN2_MNEMONIC}"
"${ADMIN3_MNEMONIC}"
...
VAL_NODE_NUM=${#FBCHAIN_DEVNET_VAL_ADMIN_MNEMONIC[@]}

CHAIN_ID="fbchain-77777"
...
captain=$(fbchaincli keys show captain | jq -r '.address')
admin18=$(fbchaincli keys show admin18 | jq -r '.address')
admin17=$(fbchaincli keys show admin17 | jq -r '.address')
captain0x=$(fbchaincli keys show captain | jq -r '.eth_address')
admin180x=$(fbchaincli keys show admin18 | jq -r '.eth_address')
admin170x=$(fbchaincli keys show admin17 | jq -r '.eth_address')

proposal_deposit="100okt"

if [[ $CHAIN_ID == "fbchain-77777" ]];
then
  for ((i=0; i<${VAL_NODE_NUM}; i++))
  do
    mnemonic=${FBCHAIN_DEVNET_VAL_ADMIN_MNEMONIC[i]}
    res=$(fbchaincli keys add --recover val"${i}" -m "$mnemonic" -y)
  done
  val0=$(fbchaincli keys show val0 -a)
  res=$(fbchaincli tx send $val0 $admin17 100fibo --from val0 $TX_EXTRA)
  res=$(fbchaincli tx send $val0 $admin18 100fibo --from val0 $TX_EXTRA)
  res=$(fbchaincli tx send $val0 $captain 100fibo --from val0 $TX_EXTRA)
fi;
...
res=$(fbchaincli tx wasm store ./wasm/cw20-base/artifacts/cw20_base.wasm --instantiate-only-address="${captain0x}" --from captain $TX_EXTRA)
...
res=$(fbchaincli tx wasm instantiate "$cw20_code_id2" '{"decimals":10,"initial_balances":[{"address":"'$captain0x'","amount":"100000000"}],"name":"my test token", "symbol":"mtt"}' --label test1 --admin "$captain" --from captain $TX_EXTRA)
...
res=$(fbchaincli tx wasm instantiate "$cw20_code_id2" '{"decimals":10,"initial_balances":[{"address":"'$captain0x'","amount":"100000000"}],"name":"my test token", "symbol":"mtt"}' --label test1 --admin "$captain" --from admin18 $TX_EXTRA)
raw_log=$(echo "$res" | jq '.raw_log' | sed 's/\"//g')
...
echo "## instantiate only address..."
res=$(fbchaincli tx wasm instantiate "$cw20_code_id3" '{"decimals":10,"initial_balances":[{"address":"'$captain0x'","amount":"100000000"}],"name":"my test token", "symbol":"mtt"}' --label test1 --admin "$captain" --from captain $TX_EXTRA)
echo "instantiate cw20 succeed"
if [[ $(echo "$res" | jq '.logs[0].events[0].attributes[0].key' | sed 's/\"//g') != "_contract_address" ]];
then
  echo "unexpected result of instantiate"
...
res=$(fbchaincli tx wasm instantiate "$cw20_code_id3" '{"decimals":10,"initial_balances":[{"address":"'$captain0x'","amount":"100000000"}],"name":"my test token", "symbol":"mtt"}' --label test1 --admin "$captain" --from admin18 $TX_EXTRA)
raw_log=$(echo "$res" | jq '.raw_log' | sed 's/\"//g')
failed_log="unauthorized: can not instantiate: failed to execute message; message index: 0"
if [[ "${raw_log}" != "${failed_log}" ]];
then
...
res=$(fbchaincli tx wasm instantiate "$cw20_code_id2" '{"decimals":10,"initial_balances":[{"address":"'$captain0x'","amount":"100000000"}],"name":"my test token", "symbol":"mtt"}' --label test1 --admin "$captain0x" --from captain $TX_EXTRA)
echo "instantiate cw20 succeed"
if [[ $(echo "$res" | jq '.logs[0].events[0].attributes[0].key' | sed 's/\"//g') != "_contract_address" ]];
then
  echo "unexpected result of instantiate"
...
depositAmount="20"
depositDenom="okt"
res=$(fbchaincli tx wasm instantiate "$cw20_code_id5" '{"decimals":10,"initial_balances":[{"address":"'$captain0x'","amount":"'${totalAmount}'"}],"name":"my test token", "symbol":"mtt"}' --label test1 --admin "$captain" --amount=${depositAmount}${depositDenom} --from captain $TX_EXTRA)
echo "instantiate cw20 succeed"
if [[ $(echo "$res" | jq '.logs[0].events[0].attributes[0].key' | sed 's/\"//g') != "_contract_address" ]];
then
...
cw20_balance=$(fbchaincli query wasm contract-state smart "$cw20contractAddr" '{"balance":{"address":"'$captain0x'"}}' $QUERY_EXTRA | jq '.data.balance' | sed 's/\"//g')
...
addr=$(fbchaincli query wasm contract-state smart "$cw4contractAddr" '{"staked":{"address":"'$captain0x'"}}' $QUERY_EXTRA | jq '.data.denom.cw20' | sed 's/\"//g')
...
cw4stake=$(fbchaincli query wasm contract-state smart "$cw4contractAddr" '{"staked":{"address":"'$captain0x'"}}' $QUERY_EXTRA | jq '.data.stake' | sed 's/\"//g')
...
echo "## update admin..."
res=$(fbchaincli tx wasm set-contract-admin "$cw4contractAddr" "$admin180x" --from admin17 $TX_EXTRA)
raw_log=$(echo "$res" | jq '.raw_log' | sed 's/\"//g')
failed_log="unauthorized: can not modify contract: failed to execute message; message index: 0"
if [[ "${raw_log}" != "${failed_log}" ]];
then
...
res=$(fbchaincli query wasm contract "$cw4contractAddr" $QUERY_EXTRA | jq -r '.contract_info.admin')
if [[ $res != $admin170x ]];
then
...
echo "## clear admin..."
res=$(fbchaincli tx wasm clear-contract-admin "$cw4contractAddr" --from admin17 $TX_EXTRA)
actionName=$(echo "$res" | jq '.logs[0].events[0].attributes[0].value' | sed 's/\"//g')
...
res=$(fbchaincli tx wasm instantiate "$cw20_code_id" '{"decimals":10,"initial_balances":[{"address":"'"$captain0x"'","amount":"'$totalAmount'"}],"name":"my test token", "symbol":"mtt"}' --label test1 --admin "$captain" --from captain $TX_EXTRA)
...
cw20contractAddr=$(echo "$res" | jq '.logs[0].events[0].attributes[0].value' | sed 's/\"//g')
echo "cw20 contract address: $cw20contractAddr"
balance=$(fbchaincli query wasm contract-state smart "$cw20contractAddr" '{"balance":{"address":"'$captain0x'"}}' "$QUERY_EXTRA" | jq '.data.balance' | sed 's/\"//g')
...
echo "transfer cw20..."
res=$(fbchaincli tx wasm execute "$cw20contractAddr" '{"transfer":{"amount":"'$transferAmount'","recipient":"'$admin180x'"}}' --from captain $TX_EXTRA)
balance=$(fbchaincli query wasm contract-state smart "$cw20contractAddr" '{"balance":{"address":"'$captain0x'"}}' "$QUERY_EXTRA" | jq '.data.balance' | sed 's/\"//g')
...
balance=$(fbchaincli query wasm contract-state smart "$cw20contractAddr" '{"balance":{"address":"'$admin180x'"}}' "$QUERY_EXTRA" | jq '.data.balance' | sed 's/\"//g')
...
stake=$(fbchaincli query wasm contract-state smart "$contractAddr" '{"staked":{"address":"'$captain0x'"}}' "$QUERY_EXTRA" | jq '.data.stake' | sed 's/\"//g')
if [[ $stake != $(($transferAmount)) ]];
then
  echo "unexpected stake after send"
  exit 1
fi;
weight=$(fbchaincli query wasm contract-state smart "$contractAddr" '{"member":{"addr":"'$captain0x'"}}' "$QUERY_EXTRA" | jq '.data.weight' | sed 's/\"//g')
...
cw4contractAddr=$(echo "$res" | jq '.logs[0].events[0].attributes[0].value' | sed 's/\"//g')
echo "cw4-stake contractAddr: $cw4contractAddr"
denom=$(fbchaincli query wasm contract-state smart "$cw4contractAddr" '{"staked":{"address":"'$captain0x'"}}' $QUERY_EXTRA | jq '.data.denom.native' | sed 's/\"//g')
if [[ $denom != "okt" ]];
then
...
stake=$(fbchaincli query wasm contract-state smart "$cw4contractAddr" '{"staked":{"address":"'$captain0x'"}}' $QUERY_EXTRA | jq '.data.stake' | sed 's/\"//g')
if [[ $stake != $amount ]];
then
