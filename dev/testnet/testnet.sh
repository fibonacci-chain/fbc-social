...
run() {
  ...
  fbchaind add-genesis-account 0x28A9Bc244A5776cDAeFf6E646f0a368BAb344BD2 900000000fibo --home cache/node${index}/fbchaind
  fbchaind add-genesis-account 0x839868517f52263023F63222854698ceFD5D7217 900000000fibo --home cache/node${index}/fbchaind
  fbchaind add-genesis-account 0x9a80eB69b1E978474bF6b217B086271bD2C66923 900000000fibo --home cache/node${index}/fbchaind
  fbchaind add-genesis-account 0x692b5C42d871675B442e5CFc169e3A41d3F81620 900000000fibo --home cache/node${index}/fbchaind
  fbchaind add-genesis-account 0xE62209Dbf1a471804426B4E584eb28E8BD867AA5 900000000fibo --home cache/node${index}/fbchaind

#  LOG_LEVEL=main:info,*:error,consensus:error,state:info
  LOG_LEVEL=main:info,iavl:info,*:error,state:info,provider:info,consensus:error,x/mint:debug
